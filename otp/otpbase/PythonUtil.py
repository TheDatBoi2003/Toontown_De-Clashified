...
def fitSrcAngle2Dest(src, dest):
    """
    given a src and destination angle, returns an equintervalent src angle
...
def fitDestAngle2Src(src, dest):
    """
    given a src and destination angle, returns an equintervalent dest angle
...
def lineTag(baseFileName=1, verbose=0, separator=':'):
    ...
    (equintervalent to lineInfo, above, with different return type)
...
def choice(condition, ifTrue, ifFalse):
    # equintervalent of C++ (condition ? ifTrue : ifFalse)
    if condition:
        return ifTrue
    else:
        return ifFalse
