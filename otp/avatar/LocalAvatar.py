...
from otp.chat import ToontownControlManager
...
class LocalAvatar(DistributedAvatar.DistributedAvatar, DistributedSmoothNode.DistributedSmoothNode):
    ...
    def setupControls(self, avatarRadius=1.4, floorOffset=OTPGlobals.FloorOffset, reach=4.0,
                      wallBitmask=OTPGlobals.WallBitmask, floorBitmask=OTPGlobals.FloorBitmask,
    ...
    def initCameraPositions(self):
        ...
        scXoffset = 3.0
        self.cameraPositions = [(Point3(0.0, -3.0 * camHeight, camHeight),
                                 defLookAt,
                                 Point3(0.0, camHeight, camHeight * 4.0),
                                 Point3(0.0, camHeight, camHeight * -1.0),
                                 0),
                                (Point3(0.0, -8.0 * camHeight, camHeight + 4.0),
                                 defLookAt,
                                 Point3(0.0, 1.5, camHeight * 4.0),
                                 Point3(0.0, 1.5, camHeight * -1.0),
                                 0),
                                (Point3(0.0, -4.0 * camHeight, camHeight + 4.0),
                                 defLookAt,
                                 Point3(0.0, 1.5, camHeight * 4.0),
                                 Point3(0.0, 1.5, camHeight * -1.0),
    ...
    def updateSmartCamera(self, task):
        if base.wantNewCamera and base.localAvatar.cameraFSM.currentCamera:
            return Task.cont
    ...
    def lerpCameraFov(self, fov, time):
        ...
        if abs(fov - oldFov) > 0.1:
            def setCamFov(fov):
                base.camLens.setMinFov(fov / base.getAspectRatio())
    ...
    def setCameraFov(self, fov):
        ...
        if not (self.isPageDown or self.isPageUp):
            base.camLens.setMinFov(self.fov / base.getAspectRatio())
    ...
    def sleepSwimTest(self, task):
        ...
        if speed != 0.0 or rotSpeed != 0.0 or slideSpeed != 0.0 or inputState.isSet('jump'):
            ...
            if (slideSpeed < 0 and speed >= 0) or (slideSpeed > 0 and speed < 0.0):
                needH = 45 * ((speed == 0.0) + 1)
            elif (slideSpeed > 0 and speed >= 0) or (slideSpeed < 0 and speed < 0.0):
                needH = -45 * ((speed == 0.0) + 1)
            else:
                needH = 0
            if self.lastNeedH != needH:
                rotLerp = Sequence(LerpHprInterval(node, 0.07, Vec3(needH, 0, 0), blendType='easeInOut'))
                rotLerp.start()
    ...
    def trackAnimToSpeed(self, task):
        ...
        if speed != 0.0 or rotSpeed != 0.0 or slideSpeed != 0.0 or inputState.isSet('jump'):
            if not self.movingFlag:
                self.movingFlag = 1
                self.stopLookAround()
            if not self.controlManager.fullTurning:
                needH = 0
                turnSpeed = 0.07
                if (slideSpeed < 0 and speed >= 0) or (slideSpeed > 0 and speed < 0.0):
                    needH = 45 * ((speed == 0.0) + 1)
                ...
                if slideSpeed < 0:
                    ...
                    needH = 180
                if self.lastNeedH:
                    if self.lastNeedH > 90 and needH < -90:
                        node.setH(self.lastNeedH - 360)
                    elif self.lastNeedH < -90 and needH > 90:
                        node.setH(self.lastNeedH + 360)
            if self.lastNeedH != needH:
                rotLerp = Sequence(LerpHprInterval(node, turnSpeed, Vec3(needH, 0, 0), blendType='easeInOut'))
                rotLerp.start()
                self.lastNeedH = needH
        ...
        if self.cheesyEffect == OTPGlobals.CEFlatProfile or self.cheesyEffect == OTPGlobals.CEFlatPortrait:
            ...
            if rotSpeed > 0.0:
                ...
                needH = 0
            if needH and self.lastNeedH != needH:
                flatLerp = Sequence(LerpHprInterval(node, 0.5, Vec3(needH, 0, 0), blendType='easeInOut'),
                                    name='cheesy-lerp-hpr', autoPause=1)
                flatLerp.start()
                self.lastNeedH = needH
        elif speed == 0 and slideSpeed == 0:
            self.lastNeedH = None
    ...
    def returnToNormal(self, immediate=0):
        if not self.getGeomNode():
            return
        node = self.getGeomNode().getChild(0)
        if not immediate:
            rotLerp = Sequence(LerpHprInterval(node, 0.07, Vec3(0, 0, 0), blendType='easeInOut'))
            rotLerp.start()
        else:
            node.setHpr(0, 0, 0)
    ...
    def __friendOnline(self, doId, commonChatFlags=0, whitelistChatFlags=0, alert=True):
        ...
        if friend and hasattr(friend, 'setCommonAndWhitelistChatFlags'):
            friend.setCommonAndWhitelistChatFlags(commonChatFlags, whitelistChatFlags)
        if self.oldFriendsList:
            now = globalClock.getFrameTime()
