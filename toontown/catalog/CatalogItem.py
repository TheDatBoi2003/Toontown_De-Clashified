...
class CatalogItem:
    ...
    def formatOptionalData(self, store = -1):
        result = ''
        if store & Location and self.posHpr:
    ...
    def applyColor(self, model, colorDesc):
        if model is None or colorDesc is None:
            return
    ...
    def makeFrameModel(self, model, spin = 1):
        ...
        if model:
            ...
            if spin:
                ...
                pitch.setY(2)
                interval = LerpHprInterval(rotate, 10, VBase3(-270, 0, 0), startHpr=VBase3(90, 0, 0))
            else:
