...
class CatalogItemPanel(DirectFrame):
    ...
    def load(self):
        ...
        if self['item'].needsCustomize() and (typeCode == CatalogItemTypes.WALLPAPER_ITEM or typeCode == CatalogItemTypes.FLOORING_ITEM or typeCode == CatalogItemTypes.MOULDING_ITEM or typeCode == CatalogItemTypes.FURNITURE_ITEM or typeCode == CatalogItemTypes.WAINSCOTING_ITEM or typeCode == CatalogItemTypes.TOON_STATUE_ITEM):
            ...
            picture, self.interval = self['item'].getPicture(base.localAvatar)
            if picture:
                picture.reparentTo(self)
                picture.setScale(0.15)
            self.items = [self['item']]
            self.variantPictures = [(picture, self.interval)]
    ...
    def showCurrentVariant(self):
        newPic, self.interval = self.variantPictures[self.itemIndex]
        if self.interval:
            self.interval.finish()
        if not newPic:
            variant = self.items[self.itemIndex]
            newPic, self.interval = variant.getPicture(base.localAvatar)
            self.variantPictures[self.itemIndex] = (newPic, self.interval)
    ...
    def unload(self):
        ...
        for picture, interval in self.variantPictures:
            if picture:
                picture.destroy()
            if interval:
                interval.finish()
    ...
    def handleSoundOnButton(self):
        ...
        if hasattr(item, 'changeIval'):
            if self.interval:
                self.interval.finish()
                self.interval = None
            self.interval = item.changeIval(volume=1)
            self.interval.loop()
        return
    ...
    def handleSoundOffButton(self):
        ...
        if hasattr(item, 'changeIval'):
            if self.interval:
                self.interval.finish()
                self.interval = None
            self.interval = item.changeIval(volume=0)
            self.interval.loop()
        return
