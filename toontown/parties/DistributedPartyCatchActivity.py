from panda3d.core import Vec3, Point3, Point4, TextNode, NodePath, CollisionHandlerEvent, CollisionNode, CollisionSphere
...
class DistributedPartyCatchActivity(DistributedPartyActivity, DistributedPartyCatchActivityBase):
    ...
    def getDropIval(self, x, y, dropObjName, generation, num):
        ...
        interval = Sequence(Func(Functor(dropNode.reparentTo, self.root)), Parallel(Sequence(WaitInterval(self.OffscreenTime), Func(Functor(object.reparentTo, dropNode)), dropIval), shadowIval), Func(cleanup), name='drop%s' % (id,))
        if objType == PartyGlobals.Name2DropObjectType['anvil']:
            interval.append(Func(self.playAnvil))
        return interval
