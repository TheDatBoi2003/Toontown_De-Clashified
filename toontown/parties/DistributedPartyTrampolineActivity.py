...
from panda3d.core import CardMaker, NodePath, TextNode, Point3, Vec3, VBase3, VBase4, CollisionSphere, CollisionTube, CollisionNode, BitMask32
...
class DistributedPartyTrampolineActivity(DistributedPartyActivity):
    ...
    def loadModels(self):
        ...
        self.beans = [ loader.loadModel('phase_4/models/props/jellybean4') for i in xrange(self.numJellyBeans) ]
    ...
    def leaveTrampoline(self):
        if self.toon and self.toon.doId == base.localAvatar.doId:
    ...
    def onTrampolineTrigger(self, collEntry):
        if self.activityFSM.state == 'Idle' and self.toon is None and base.cr.playGame.getPlace().fsm.getCurrentState().getName() == 'walk':
    ...
    def onJump(self):
        ...
        if self.toon and self.toon.getZ() < self.trampHeight:
