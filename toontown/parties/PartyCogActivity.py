...
class PartyCogActivity(DirectObject):
    ...
    def _destroyArenaDoors(self):
        for interval in self._arenaDoorIvals:
            interval.finish()
    ...
    def _playArenaDoorIval(self, team, opening = True):
        interval = self._arenaDoorIvals[team]
        if interval is not None and interval.isPlaying():
            interval.pause()
        if not opening:
        ...
        interval = self._arenaDoors[team].posInterval(0.75, pos, blendType='easeIn')
        self._arenaDoorIvals[team] = interval
        interval.start()
        return
    ...
    def finishPieIvals(self, toonId):
        for interval in self.pieIvals:
            if interval.isPlaying():
                if interval.getName().find(str(toonId)) != -1:
                    interval.finish()
    ...
    def playPlayerEnterIval(self):
        ...
        interval = Sequence(Func(self.disableWallCollisions), Func(conditionallyShowSwitchButton, self, False), self.player.getRunToStartPositionIval(), Func(conditionallyShowSwitchButton, self, True), Func(self.enableWallCollisions))
        self.playToonIval(base.localAvatar.doId, interval)
    ...
    def playToonIval(self, toonId, interval):
        self.finishToonIval(toonId)
        self.toonIdsToAnimIntervals[toonId] = interval
        interval.start()
