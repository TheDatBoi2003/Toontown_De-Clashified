...
class LawbotHQDataAI(HoodDataAI.HoodDataAI):
    ...
    def startup(self):
        ...
        def makeOfficeElevator(index):
            ...
            elev = DistributedLawOfficeElevatorExtAI.DistributedLawOfficeElevatorExtAI(self.air, self.air.lawMgr, destZone, index)
            elev.generateWithRequired(ToontownGlobals.LawbotOfficeExt)
            self.addDistObj(elev)
            return elev.doId

        officeId0 = makeOfficeElevator(0)
        officeId1 = makeOfficeElevator(1)
        officeId2 = makeOfficeElevator(2)
        officeId3 = makeOfficeElevator(3)
