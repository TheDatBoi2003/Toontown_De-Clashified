...
def clearPythonIvals(interval):
    if hasattr(interval, 'function'):
        interval.function = None
    if hasattr(interval, 'pythonIvals'):
        for oneIval in interval.pythonIvals:
            clearPythonIvals(oneIval)

        interval.pythonIvals = []
    return
...
class InteractiveAnimatedProp(GenericAnimatedProp.GenericAnimatedProp, FSM.FSM):
    ...
    def setupActor(self, node):
        ...
        self.mainActor = Actor.Actor(node, copy=0)
        self.mainActor.reparentTo(node)
        animDict = {}
        ...
        if self.hoodId in self.ZoneToSadAnims:
            ...
            animKey = 'sad'
            animDict[animKey] = animStr
        self.mainActor.loadAnims(animDict)
        self.mainActor.pose('anim', 0)
        self.node = self.mainActor
    ...
    def createBattleCheerInterval(self):
        ...
        intervalWithText = Parallel(battleCheerTextIval, result)
        return intervalWithText
    ...
    def calcLastIdleFrame(self):
        if self.curIval and self.curIval.intervals:
            firstIval = self.curIval.intervals[0]
            if isinstance(firstIval, ActorInterval):
                ...
                for testIval in firstIval.intervals:
                    if isinstance(firstIval, ActorInterval):
