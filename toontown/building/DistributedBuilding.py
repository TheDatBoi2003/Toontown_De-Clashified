...
class DistributedBuilding(DistributedObject.DistributedObject):
    ...
    def unloadSfx(self):
        if self.cogDropSound:
            self.cogDropSound = None
            self.cogLandSound = None
            self.cogSettleSound = None
            self.openSfx = None
        if self.cogWeakenSound:
            self.cogWeakenSound = None
            self.toonGrowSound = None
            self.toonSettleSound = None
            self.openSfx = None
    ...
    def setupSuitBuilding(self, nodePath):
        ...
        building = 'suit_landmark_' + chr(self.track) + str(level)
        suitNP = dnaStore.findNode(building)
        if not suitNP:
            self.notify.warning('Could not find building %s!' % building)
            return None
