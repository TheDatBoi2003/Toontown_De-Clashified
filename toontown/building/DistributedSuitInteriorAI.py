...
from toontown.battle import DistributedBattleBldgAI, SuitBattleGlobals
...
class DistributedSuitInteriorAI(DistributedObjectAI.DistributedObjectAI):
    ...
    def __handleRoundDone(self, toonIds, totalHp, deadSuits):
        ...
        if len(self.reserveSuits) > 0 and len(self.activeSuits) < SuitBattleGlobals.MAX_SUIT_CAPACITY:
            self.joinedReserves = []
