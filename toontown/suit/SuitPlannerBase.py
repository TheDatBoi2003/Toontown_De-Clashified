...
class SuitPlannerBase:
    ...
    SuitHoodInfo = [[2100, 5, 15, 0, 5, 20, 3,
                     (1, 5, 10, 40, 60, 80),
                     (25, 25, 25, 25),
                     (1, 2, 3),
                     []],
            ...
            if True:
                for i in xrange(vg.getNumChildren()):
                    ...
                    if isinstance(childDnaGroup, DNAInteractiveProp):
                        ...
                        if battleCellId == -1:
                            self.notify.warning(
                                'interactive prop %s  at %s not associated with a a battle' % (childDnaGroup, zoneId))
                        elif battleCellId == 0:
                            if zoneId in self.cellToGagBonusDict:
                                self.notify.error('FIXME battle cell at zone %s has two props %s %s linked to it' % (
    ...
    def performPathTest(self):
        ...
        for i in xrange(numPathPoints - 1):
            ...
            travelTime = self.dnaStore.getSuitEdgeTravelTime(path.getPointIndex(i), path.getPointIndex(i + 1),
                                                             self.suitWalkSpeed)
            self.notify.debug('edge from point ' + `i` + ' to point ' + `(
                        i + 1)` + ' is in zone: ' + `zone` + ' and will take ' + `travelTime` + ' seconds to walk.')
