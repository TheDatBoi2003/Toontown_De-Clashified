...
from toontown.battle import BattleExperienceAI, SuitBattleGlobals
...
class DistributedBossCogAI(DistributedAvatarAI.DistributedAvatarAI):
    ...
    def __init__(self, air, dept):
        ...
        self.hitCount = 0
        self.nerfed = False
        self.numRentalDisguises = 0
        self.numNormalDisguises = 0
    ...
    def b_setBattleTier(self, tier):
        self.setBattleTier(tier)
        self.d_setBattleTier(tier)

    def d_setBattleTier(self, tier):
        self.sendUpdate('setBattleTier', [tier])
    ...
    def __randomDivide(self):
        ...
        toonsToAdd = min(len(toons), 8)
        if toonsToAdd < 4:
            splitPoint = toonsToAdd
        else:
            splitPoint = toonsToAdd / 2
        teamA = toons[:splitPoint]
        teamB = toons[splitPoint:toonsToAdd]
        loose = toons[toonsToAdd:]
        return (
         teamA, teamB, loose)
    ...
    def updateBattleTier(self):
        ...
        if not self.involvedToons:
            ...
            sourceToons = self.involvedToons
        for avId in sourceToons:
            toon = self.air.doId2do.get(avId)
            if toon:
        ...
        if 0 <= suitTierAvg < 3:
            ...
            self.b_setBattleTier(1)
        else:
            self.b_setBattleTier(2)
        self.notify.info("Battle tier for current boss: " + str(self.battleTier))
