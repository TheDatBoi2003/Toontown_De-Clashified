...
class DistributedLawbotBoss(DistributedBossCog.DistributedBossCog, FSM.FSM):
    ...
    def __walkToonToPromotion(self, toonId, delay, mopath, track, delayDeletes):
        ...
        if toon:
            ...
            interval = Sequence(Wait(delay), Func(toon.suit.setPlayRate, 1, 'walk'), Func(toon.suit.loop, 'walk'), toon.posInterval(1, Point3(0, 90, 20)), ParallelEndTogether(MopathInterval(mopath, toon), toon.posInterval(2, destPos, blendType='noBlend')), Func(toon.suit.loop, 'neutral'))
    ...
    def makeIntroductionMovie(self, delayDeletes):
        ...
        attackToons = TTLocalizer.LawbotBossAttack
        dialogTrack = Track(
    ...
    def calculateWeightOfToon(self, toonId):
        ...
        numJurors = 0
        if not cannonIndex is None and cannonIndex >= 0:
            diffSettings = ToontownGlobals.LawbotBossDifficultySettings[self.battleDifficulty]
            if diffSettings[4]:
