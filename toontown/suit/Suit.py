...
HeadModelDict = {'a': ('/models/char/suitA-', 4),
                 'b': ('/models/char/suitB-', 4),
                 'c': ('/models/char/suitC-', 3.5)}
AccessoryDict = {'hat': {'models': ('/models/accessories/tt_m_chr_avt_acc_hat_', 4),
                         'maps': ('/maps/tt_t_chr_avt_acc_hat_', 4)},
                 'mask': {'models': ('/models/accessories/tt_m_chr_avt_acc_msk_', 4),
                          'maps': ('/maps/tt_t_chr_avt_acc_msk_', 4)}}
...
def loadDialog():
    ...
    if len(SuitDialogArray) > 0:
        ...
        grunt = loader.loadSfx('phase_3.5/audio/dial/COG_VO_grunt.ogg')
        murmur = loader.loadSfx('phase_3.5/audio/dial/COG_VO_murmur.ogg')
        statement = loader.loadSfx('phase_3.5/audio/dial/COG_VO_statement.ogg')
        question = loader.loadSfx('phase_3.5/audio/dial/COG_VO_question_1.ogg')
        SuitDialogArray = [grunt,
                           murmur,
                           statement,
                           question,
                           grunt,
...
class Suit(Avatar.Avatar):
    ...
    def generateSuit(self):
        ...
        if dna.name == 'f':
            ...
            self.handColor = VBase4(0.98, 0.89, 0.82, 1.0)
            self.generateBody()
            self.generateHead('telemarketer')
            self.setHeight(5.24)
        elif dna.name == 'nd':
            self.scale = 4.35 / aSize
            self.handColor = VBase4(0.78, 0.655, 0.84, 1.0)
            self.generateBody()
            ...
            self.scale = 5.25 / aSize
            self.handColor = VBase4(0.98, 0.86, 0.75, 1.0)
            self.generateBody()
            self.generateHead('twoface')
            self.setHeight(6.95)
        elif dna.name == 'm':
            ...
            self.handColor = VBase4(1.0, 0.55, 0.65, 1.0)
            self.headColor = VBase4(1.0, 0.25, 0.35, 1.0)
            self.generateBody()
            ...
            self.handColor = VBase4(0.97, 0.86, 0.62, 1.0)
            self.generateBody()
    ...
    def setSuitClothes(self, modelRoot=None):
        ...
        if base.config.GetBool('want-new-cogs', 0):
            texType = getTexType(dept)
            if self.find('**/body').isEmpty():
                __doItTheOldWay__()
            else:
                self.setBodyTexture(texType)
    ...
    def setBodyTexture(self, texType):
        filepath = 'phase_3.5/maps/tt_t_ene_' + texType + '.jpg'
        if cogExists('/maps/' + filepath):
    ...
    def generateHead(self, headType, bodyType=None):
        if not bodyType:
            bodyType = self.style.body
        if base.config.GetBool('want-new-cogs', 0):
            filePrefix, phase = HeadModelDict[bodyType]
        else:
            filePrefix, phase = ModelDict[bodyType]
    ...
    def generateAccessory(self, accName, texName=None, accType='hat', transforms=None):
        filePrefix, phase = AccessoryDict[accType]['models']
        accModel = loader.loadModel('phase_' + str(phase) + filePrefix + accName + '.bam')
        accPart = self.instance(accModel, 'modelRoot', 'joint_head')
        if not texName:
            texName = accName
        ...
        if transforms:
            ...
            accPart.setHpr(transforms[1][0], transforms[1][1], transforms[1][2])
            accPart.setScale(transforms[2][0], transforms[2][1], transforms[2][2])
        self.headParts.append(accPart)
    ...
    # NOTE: DO NOT CALL THIS OR SOME HEALTH BARS WILL BREAK!
    def updateHealthBar(self, hp=0, forceUpdate=0):
        immunity = 'immune' in self.statuses
        if hp > 0 and not immunity:
            self.currHP -= hp
        health = float(self.currHP) / float(self.maxHP)
        self.notify.info("%s: %s/%s: (immune: %s)" % (self.dna.name, self.currHP, self.maxHP, immunity))
        if 1 >= health > 0.95:
            condition = 0
        elif 0.95 >= health > 0.7:
            condition = 1
        elif 0.7 >= health > 0.3:
            condition = 2
        elif 0.3 >= health > 0.05:
            condition = 3
        elif 0.05 >= health > 0.0:
            condition = 4
        elif 0.0 >= health:
            condition = 5
        else:
    ...
    def getLoseActor(self, headless=0):
        ...
        if self.loseActor is None:
            if not self.isSkeleton:
                ...
                self.setExecColor(self.loseActor)
    ...
    def setExecColor(self, modelRoot=None):
        if not modelRoot:
            modelRoot = self
        if self.isExecutive:
            if self.dna.dept == 'c':
