...
class DistributedCashbotBossGoonAI(DistributedGoonAI.DistributedGoonAI, DistributedCashbotBossObjectAI.DistributedCashbotBossObjectAI):
    ...
    def __chooseTarget(self, extraDelay = 0):
        ...
        self.arrintervalTime = self.departureTime + turnTime + walkTime + extraDelay
        self.d_setTarget(self.target[0], self.target[1], h, globalClockDelta.localToNetworkTime(self.arrintervalTime))
        return
    ...
    def __startWalk(self):
        if self.arrintervalTime is None:
            return
        now = globalClock.getFrameTime()
        availableTime = self.arrintervalTime - now
        if availableTime > 0:
    ...
    def __stopWalk(self, pauseTime = None):
        if self.isWalking:
            ...
            if pauseTime is None:
                now = globalClock.getFrameTime()
                t = (now - self.departureTime) / (self.arrintervalTime - self.departureTime)
            else:
                t = pauseTime / (self.arrintervalTime - self.departureTime)
            t = min(t, 1.0)
            pos = self.getPos()
    ...
    def d_setTarget(self, x, y, h, arrintervalTime):
    ...
    def enterEmergeA(self):
        ...
        self.target = self.boss.scene.getRelativePoint(self, Point3(0, dist, 0))
        self.departureTime = globalClock.getFrameTime()
        self.arrintervalTime = self.departureTime + walkTime
        self.d_setTarget(self.target[0], self.target[1], h, globalClockDelta.localToNetworkTime(self.arrintervalTime))
        self.__startWalk()
