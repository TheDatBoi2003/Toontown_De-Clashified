...
class DistributedBossCog(DistributedAvatar.DistributedAvatar, BossCog.BossCog):
    ...
    def announceGenerate(self):
        ...
        self.bubbleR.stash()
        self.bubbleS = [None, None, None, None]
        spinPositions = ((0, -25), (25, 0), (0, 25), (-25, 0))
        for i in xrange(4):
            bubbleS = CollisionSphere(spinPositions[i][0], spinPositions[i][1], 0, 10)
            bubbleS.setTangible(0)
            bubbleSNode = CollisionNode('BossZap')
            bubbleSNode.setCollideMask(ToontownGlobals.WallBitmask)
            bubbleSNode.addSolid(bubbleS)
            self.bubbleS[i] = self.rotateNode.attachNewNode(bubbleSNode)
            self.bubbleS[i].setTag('attackCode', str(ToontownGlobals.BossCogSpinAttack))
            self.bubbleS[i].stash()
    ...
    def getDialogueArray(self, *args):
        return [self.grunt, self.murmur, self.statement, self.question, self.grunt, self.murmur]
    ...
    def setBattleTier(self, tier):
        self.battleTier = tier
    ...
    def setAttackCode(self, attackCode, avId = 0):
        ...
        elif attackCode == ToontownGlobals.BossCogSpinAttack:
            self.setDizzy(0)
            self.doAnimate('spinAttack', now=1)
    ...
    def backupToonsToBattlePosition(self, toonIds, battleNode):
        ...
        for i in xrange(len(toonIds)):
            ...
            if toon:
                ...
                interval.append(Sequence(Func(toon.setPlayRate, -0.8, 'walk'), Func(toon.loop, 'walk'), toon.posInterval(3, pos), Func(toon.setPlayRate, 1, 'walk'), Func(toon.loop, 'neutral')))
    ...
    def announceAreaAttack(self):
        self.endAreaAttack()
        taskMgr.add(self._areaAttackTask, '%s-AreaAttack' % self.name)

    def endAreaAttack(self):
        taskMgr.remove('%s-AreaAttack' % self.name)

    def _areaAttackTask(self, task):
    ...
    def enterElevator(self):
        ...
        interval = Sequence(ElevatorUtils.getRideElevatorInterval(self.elevatorType), ElevatorUtils.getRideElevatorInterval(self.elevatorType), self.openDoors, Func(camera.wrtReparentTo, render), Func(self.__doneElevator))
