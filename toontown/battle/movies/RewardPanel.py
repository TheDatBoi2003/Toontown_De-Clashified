...
class RewardPanel(DirectFrame):
    ...
    def __init__(self, name):
        ...
        DirectFrame.__init__(self, relief=None, geom=DGG.getDefaultDialogGeom(),
                             geom_color=ToontownGlobals.GlobalDialogColor, geom_pos=Point3(0, 0, -.05),
                             geom_scale=gscale, pos=(0, 0, 0.587))
        ...
        self.itemFrame = DirectFrame(parent=self, relief=None, text=TTLocalizer.RewardPanelItems, text_pos=(0, 0.2),
                                     text_scale=0.08)
        self.cogPartFrame = DirectFrame(parent=self, relief=None, text=TTLocalizer.RewardPanelCogPart,
                                        text_pos=(0, 0.2), text_scale=0.08)
        self.missedItemFrame = DirectFrame(parent=self, relief=None, text=TTLocalizer.RewardPanelMissedItems,
                                           text_pos=(0, 0.2), text_scale=0.08)
        ...
        self.questFrame = DirectFrame(parent=self, relief=None, text=TTLocalizer.RewardPanelToonTasks,
        ...
        self.newGagFrame = DirectFrame(parent=self, relief=None, pos=(0, 0, 0.24), text='', text_wordwrap=14.4,
                                       text_pos=(0, -0.46), text_scale=0.06)
        self.endTrackFrame = DirectFrame(parent=self, relief=None, pos=(0, 0, 0.24), text='', text_wordwrap=14.4,
                                         text_pos=(0, -0.46), text_scale=0.06)
        self.congratsLeft = DirectLabel(parent=self.newGagFrame, pos=(-0.2, 0, -0.1), text='', text_pos=(0, 0),
                                        text_scale=0.06)
        self.congratsLeft.setHpr(0, 0, -30)
        self.congratsRight = DirectLabel(parent=self.newGagFrame, pos=(0.2, 0, -0.1), text='', text_pos=(0, 0),
                                         text_scale=0.06)
        self.congratsRight.setHpr(0, 0, 30)
        self.promotionFrame = DirectFrame(parent=self, relief=None, pos=(0, 0, 0.24), text='', text_wordwrap=14.4,
                                          text_pos=(0, -0.46), text_scale=0.06)
        self.trackLabels = []
        self.trackIncLabels = []
        self.trackBars = []
        ...
        for i in xrange(len(ToontownBattleGlobals.Tracks)):
            ...
            self.trackLabels.append(
                DirectLabel(parent=self.gagExpFrame, relief=None, text=trackName, text_scale=TTLocalizer.RPtrackLabels,
                            text_align=TextNode.ARight, pos=(0.13, 0, -0.09 * i), text_pos=(0, -0.02)))
        ...
        self.skipButton = DirectButton(parent=self, relief=None,
                                       ...
                                       pos=(0.815, 0, -0.395), scale=(0.39, 1.0, 0.39), text=('',
                                                                                              TTLocalizer.RewardPanelSkip,
                                                                                              TTLocalizer.RewardPanelSkip,
                                                                                              ''),
                                       text_scale=TTLocalizer.RPskipScale, text_fg=Vec4(1, 1, 1, 1),
                                       text_shadow=Vec4(0, 0, 0, 1), text_pos=TTLocalizer.RPskipPos, textMayChange=0,
        ...
        for i in xrange(len(expList)):
            ...
            if toon.hasTrackAccess(i):
                ...
                if curExp >= ToontownBattleGlobals.MaxSkill:
                    trackBar['range'] = 1
                    trackBar['value'] = 1
                    trackBar['text'] = TTLocalizer.InventoryMax
                else:
    ...
    def incrementExp(self, track, newValue, toon):
        ...
        if newValue >= ToontownBattleGlobals.MaxSkill:
            ...
            trackBar['text'] = TTLocalizer.InventoryMaxOut
        else:
    ...
    def uberGagInterval(self, toon, track, level):
        ...
        self.itemFrame.hide()
        self.missedItemFrame.hide()
        self.newGagFrame['text'] = TTLocalizer.RewardPanelEndTrack % {
    ...
    def endTrack(self, toon, toonList, track):
        ...
        self.endTrackFrame['text'] = TTLocalizer.RewardPanelEndTrack % {
            'gagName': ToontownBattleGlobals.Tracks[track].capitalize(),
            'avName': toon.getName()}
        gagLast = base.localAvatar.inventory.buttonLookup(track, ToontownBattleGlobals.MAX_LEVEL_INDEX)
    ...
    def getTrackIntervalList(self, toon, track, origSkill, earnedSkill):
        tickDelay = 1.0 / 60
        intervalList = [Func(self.showTrackIncLabel, track, earnedSkill)]
        barTime = 0.5
