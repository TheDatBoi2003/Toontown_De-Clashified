...
def doLures(lures):
    if len(lures) == 0:
        return None, None
    npcArrintervals, npcDepartures, npcToons = MovieNPCSOS.doNPCTeleports(lures)
    mainTrack = Parallel()
    for lure in lures:
        lureFunct = __doLureLevel(lure, npcToons)
        if lureFunct:
            mainTrack.append(lureFunct)

    lureTrack = Sequence(npcArrintervals, mainTrack, npcDepartures)
...
def __doLureLevel(lure, npcToons):
    level = lure['level']
    if level == 0:
        return __lureDollar(lure)
    elif level == 1:
        return __lureSmallMagnet(lure, npcToons)
    elif level == 2:
        return __lureFiveDollar(lure)
    elif level == 3:
        return __lureLargeMagnet(lure, npcToons)
    elif level == 4:
        return __lureTenDollar(lure)
    elif level == 5:
        return __lureHypnotize(lure, npcToons)
    elif level == 6:
        return __lureFiftyDollar(lure)
    elif level == 7:
        return __lureSlideshow(lure, npcToons)
    return None
...
def __createFishingPoleMultiTrack(lure, dollar, dollarName):
    ...
    dollarTrack = Sequence(Func(positionDollar), Func(dollar.wrtReparentTo, battle),
                           ActorInterval(dollar, dollarName, duration=3),
                           getSplicedLerpAnimsTrack(dollar, dollarName, 0.7, 2.0, startTime=3),
                           LerpPosInterval(dollar, 0.2, Point3(0, -10, 7)), Func(MovieUtil.removeProp, dollar))
    poleTrack = Sequence(Func(MovieUtil.showProps, poles, hands), ActorInterval(pole, 'fishing-pole'),
                         Func(MovieUtil.removeProps, poles))
    toonTrack = Sequence(Func(toon.headsUp, battle, targetPos), ActorInterval(toon, 'battlecast'),
                         Func(toon.loop, 'neutral'))
    tracks = Parallel(dollarTrack, poleTrack, toonTrack)
    if not sidestep:
        if target['kbBonus'] or hp > 0:
            suitTrack = Sequence()
            origPos, origHpr = battle.getActorPosHpr(suit)
            reachDist = MovieUtil.SUIT_LURE_DISTANCE
            reachPos = Point3(origPos[0], origPos[1] - reachDist, origPos[2])
            suitTrack.append(Func(suit.loop, 'neutral'))
            suitTrack.append(Wait(3.5))
            __showStatusText(suit, suitTrack, trapProp)
...
def __createMagnetMultiTrack(lure, magnet, pos, hpr, scale, isSmallMagnet=1, npcToons=None):
    if npcToons is None:
        npcToons = []
    toon = lure['toon']
    if 'npc' in lure:
    ...
    magnetTrack = Sequence(Wait(0.7), Func(MovieUtil.showProps, magnets, hands, pos, hpr, scale), Wait(6.3),
    ...
    for target in targets:
        ...
        if sidestep == 0:
            ...
            if kbBonus == 1 or hp > 0:
                ...
                tracks.append(suitTrack)
                tracks.append(
                    lerpSuit(suit, suitDelay + 0.55 + shakeTotalDuration, suitMoveDuration, reachPos, battle, trapProp))
        else:
            tracks.append(Sequence(Wait(3.7), Func(MovieUtil.indicateMissed, suit)))
...
def __createHypnoGogglesMultiTrack(lure, npcToons=None):
    if npcToons is None:
        npcToons = []
    toon = lure['toon']
    if 'npc' in lure:
    ...
    hands = toon.getLeftHands()
    gogglesTrack = Sequence(Wait(0.6), Func(MovieUtil.showProps, bothGoggles, hands, pos, hpr, scale),
...
def __lureSmallMagnet(lure, npcToons=None):
    ...
    return __createMagnetMultiTrack(lure, magnet, pos, hpr, scale, isSmallMagnet=1, npcToons=npcToons)


def __lureFiveDollar(lure):
    return __lureDollar(lure, ToontownBattleGlobals.AvProps[2][2])


def __lureLargeMagnet(lure, npcToons=None):
    if npcToons is None:
        npcToons = []
    ...
    return __createMagnetMultiTrack(lure, magnet, pos, hpr, scale, isSmallMagnet=0, npcToons=npcToons)


def __lureTenDollar(lure):
    return __lureDollar(lure, ToontownBattleGlobals.AvProps[2][4])


def __lureHypnotize(lure, npcToons=None):
    if npcToons is None:
        npcToons = []
    return __createHypnoGogglesMultiTrack(lure, npcToons)


def __lureFiftyDollar(lure):
    return __lureDollar(lure, ToontownBattleGlobals.AvProps[2][6])


def __lureSlideshow(lure, npcToons):
    return __createSlideshowMultiTrack(lure, npcToons)
...
def __createSuitDamageTrack(battle, suit, hp, lure, trapProp, revived=0, died=0):
    ...
    if trapName == 'banana':
        ...
        animTrack = Sequence(ActorInterval(trapProp, 'banana', startTime=3.1), Wait(1.1),
        ...
        rakeTrack = Sequence(Wait(0.5), LerpHprInterval(trapProp, 0.1, upHpr, startHpr=hpr), Wait(0.7),
                             ...
                             LerpHprInterval(trapProp, 0.05, hpr, startHpr=bounce2Hpr), Wait(0.2),
        ...
        moveTrack = Sequence(Wait(0.1), LerpPosInterval(trapProp, 0.8, slidePos, other=battle), Wait(1.1),
                             LerpScaleInterval(trapProp, 1, Point3(0.01, 0.01, 0.01)))
        ...
        moveTrack = Sequence(Wait(0.9), LerpPosInterval(suit, 0.9, sinkPos1, other=battle),
                             LerpPosInterval(suit, 0.4, sinkPos2, other=battle), Func(suit.setPos, battle, dropPos),
                             Func(suit.wrtReparentTo, hidden), Wait(1.1))
        ...
        if died:
            ...
            animTrack.append(ActorInterval(suit, 'slip-forward', duration=2.1))
    ...
    elif trapName == 'trapdoor' or trapName == 'wreckingball':
        ...
        moveTrack = Sequence(Wait(2.2), LerpPosInterval(suit, 0.4, sinkPos, other=battle),
                             Func(suit.setPos, battle, dropPos), Func(suit.wrtReparentTo, hidden), Wait(1.6),
                             Func(suit.wrtReparentTo, battle), LerpPosInterval(suit, 0.3, landPos, other=battle))
        animTrack = Sequence(getSplicedLerpAnimsTrack(suit, 'flail', 0.7, 0.25),
                             Func(trapProp.setColor, Vec4(0, 0, 0, 1)),
                             ActorInterval(suit, 'flail', startTime=0.7, endTime=0),
                             ActorInterval(suit, 'neutral', duration=0.5), ActorInterval(suit, 'flail', startTime=1.1),
                             Wait(1.1), ActorInterval(suit, 'slip-forward', duration=2.1))
        damageTrack = Sequence(Wait(3.5), Func(suit.showHpText, -hp, openEnded=0), Func(suit.updateHealthBar, hp))
        soundTrack = Sequence(Wait(0.8), SoundInterval(globalBattleSoundCache.getSound('TL_trap_door.ogg'), node=suit),
                              Wait(0.8),
                              SoundInterval(globalBattleSoundCache.getSound('Toon_bodyfall_synergy.ogg'), node=suit))
        ...
        if died:
            suitGone = 1
            damageTrack = Sequence()
            suitTrack.append(MovieUtil.createSuitWalkOut(suit, battle))
        else:
            suitTrack2 = Sequence(Wait(0.4), ActorInterval(suit, 'slip-forward', startTime=2.48, duration=0.1),
                Func(battle.movie.needRestoreColor), Func(suit.setColorScale, Vec4(0.2, 0.2, 0.2, 1)),
                Func(trapProp.reparentTo, hidden), ActorInterval(suit, 'slip-forward', startTime=2.58),
                Func(suit.clearColorScale), Func(trapProp.sparksEffect.cleanup),
                Func(battle.movie.clearRestoreColor))
            suitTrack.append(suitTrack2)
    ...
    if suitGone and trapName == 'tnt':
        #result.append(MovieUtil.createSuitWalkOut(suit, battle))
        pass
    else:
...
def getSplicedLerpAnimsTrack(actor, animName, origDuration, newDuration, startTime=0, fps=30):
    track = Sequence()
    addition = 0
    numIntervals = origDuration * fps
    timeInterval = newDuration / numIntervals
    animInterval = origDuration / numIntervals
    for i in xrange(0, int(numIntervals)):
        track.append(Wait(timeInterval))
        track.append(ActorInterval(actor, animName, startTime=startTime + addition, duration=animInterval))
...
def lerpSuit(suit, delay, duration, reachPos, battle, trapProp):
    ...
    if trapProp:
        if trapProp.getName() == 'traintrack':
            notify.debug('UBER LURE MovieLure.lerpSuit deliberately not parenting trainTrack to suit')
        else:
...
def createSuitReactionToTrain(battle, suit, hp, lure):
    toon = lure['toon']
    retval = Sequence()
...
def createIncomingTrainInterval(battle, suit, hp, lure, trapProp):
    toon = lure['toon']
    retval = Parallel()
    suitTrack = createSuitReactionToTrain(battle, suit, hp, lure)
    retval.append(suitTrack)
    ...
    materializeInterval = Parallel()
    materializeInterval.append(LerpColorScaleInterval(train, TRAIN_MATERIALIZE_TIME, Point4(1, 1, 1, 1)))
    for tunnel in tunnels:
        materializeInterval.append(LerpColorScaleInterval(tunnel, TRAIN_MATERIALIZE_TIME, Point4(1, 1, 1, 1)))

    for tunnel in tunnels:
        tunnelScaleInterval = Sequence()
        tunnelScaleInterval.append(LerpScaleInterval(tunnel, TRAIN_MATERIALIZE_TIME - 1.0, Point3(1.0, 2.0, 2.5)))
        tunnelScaleInterval.append(LerpScaleInterval(tunnel, 0.5, Point3(1.0, 3.0, 1.5)))
        tunnelScaleInterval.append(LerpScaleInterval(tunnel, 0.5, Point3(1.0, 2.5, 2.0)))
        materializeInterval.append(tunnelScaleInterval)

    trainIval.append(materializeInterval)
...
def __createSlideshowMultiTrack(lure, npcToons=None):
    if npcToons is None:
        npcToons = []
    toon = lure['toon']
    battle = lure['battle']
...
def __showStatusText(suit, suitTrack, trapProp=0):
    if trapProp:
        suitTrack.append(Func(suit.showStatusText, SuitBattleGlobals.TRAPPED_STATUS))
    else:
        lureStatus = suit.getStatus(SuitBattleGlobals.LURED_STATUS)
        suitTrack.append(Func(suit.showStatusText, lureStatus['name'], lureStatus['rounds']))
