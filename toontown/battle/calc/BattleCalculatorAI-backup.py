...
import toontown.battle.SuitBattleGlobals, toontown.battle.BattleExperienceAI
...
class BattleCalculatorAI:
        ...
        if atkTrack == LURE:
            ...
            if self.propAndOrganicBonusStack:
                ...
                if treebonus:
                    ...
                    propAcc += AvBonusAccuracy[0][atkLevel]
                if propBonus:
    ...
    def __targetDefense(self, suit, atkTrack):
        if atkTrack == HEAL:
            return 0
        suitDef = toontown.battle.SuitBattleGlobals.SuitAttributes[suit.dna.name]['def'][suit.getLevel()]
    ...
    def __addSuitGroupTrap(self, suitId, trapLvl, attackerId, allSuits, npcDamage=0):
        if npcDamage == 0:
            if suitId in self.traps:
                ...
                prestige = toon.checkTrackPrestige(TRAP)
                propBonus = self.__checkPropBonus(TRAP)
                damage = getAvPropDamage(TRAP, trapLvl, toon.experience.getExp(TRAP), prestige, propBonus, self.propAndOrganicBonusStack)
        ...
        for currTarget in xrange(len(targetList)):
            ...
            if atkTrack == LURE:
                ...
                if attack[TOON_TRACK_COL] == NPCSOS and lureDidDamage != 1 or attack[TOON_TRACK_COL] == PETSOS:
                    ...
                    prestige = toon.checkTrackPrestige(attackTrack)
                    propBonus = self.__checkPropBonus(attackTrack)
                    attackDamage = getAvPropDamage(attackTrack, attackLevel, toon.experience.getExp(attackTrack), prestige, propBonus, self.propAndOrganicBonusStack)
                if not self.__combatantDead(targetId, toon=toonTarget):
    ...
    def __calcSuitAtkType(self, attackIndex):
        ...
        attacks = toontown.battle.SuitBattleGlobals.SuitAttributes[theSuit.dna.name]['attacks']
        atk = toontown.battle.SuitBattleGlobals.pickSuitAttack(attacks, theSuit.getLevel())
        return atk
    ...
    def __calcSuitTarget(self, attackIndex):
        ...
        if suitId in self.SuitAttackers and random.randint(0, 99) < 75:
            ...
            dmgIdx = toontown.battle.SuitBattleGlobals.pickFromFreqList(dmgs)
            if dmgIdx is None:
    ...
    def __suitAtkHit(self, attackIndex):
        ...
        atkInfo = toontown.battle.SuitBattleGlobals.getSuitAttack(theSuit.dna.name, theSuit.getLevel(), atkType)
        atkAcc = atkInfo['acc']
        suitAcc = toontown.battle.SuitBattleGlobals.SuitAttributes[theSuit.dna.name]['acc'][theSuit.getLevel()]
        acc = atkAcc
        randChoice = random.randint(0, 99)
        if self.notify.getDebug():
    ...
    def __suitAtkAffectsGroup(self, attack):
        ...
        atkInfo = toontown.battle.SuitBattleGlobals.getSuitAttack(theSuit.dna.name, theSuit.getLevel(), atkType)
        return atkInfo['group'] != toontown.battle.SuitBattleGlobals.ATK_TGT_SINGLE
    ...
    def __calcSuitAtkHp(self, attackIndex):
        ...
        for currTarget in xrange(len(targetList)):
            ...
            if toon and toon.immortalMode:
                ...
                theSuit = self.battle.findSuit(attack[SUIT_ID_COL])
                atkInfo = toontown.battle.SuitBattleGlobals.getSuitAttack(theSuit.dna.name, theSuit.getLevel(), atkType)
                result = atkInfo['hp'] + (atkInfo['hp'] * toontown.battle.SuitBattleGlobals.EXE_DMG_MOD * theSuit.isExecutive)
    ...
    def getSkillGained(self, toonId, track):
        return toontown.battle.BattleExperienceAI.getSkillGained(self.toonSkillPtsGained, toonId, track)
