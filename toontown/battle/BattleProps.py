    ...
    def __getPropCopy(self, name):
        if self.propTypes[name] == 'actor':
            if name not in self.props:
                ...
                prop.setName(name)
                prop.setBlend(frameBlend=base.settings.getBool('game', 'interpolate-animations', False))
                self.storeProp(name, prop)
                if name in Variants:
                    self.makeVariant(name)
            outProp = Actor.Actor(other=self.props[name])
            outProp.setBlend(frameBlend=base.settings.getBool('game', 'interpolate-animations', False))
            return outProp
        else:
            if name not in self.props:
