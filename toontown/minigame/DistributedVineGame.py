    ...
    def __updateOtherToonsClimbing(self):
        for avId in self.toonInfo.keys():
            ...
            if vineIndex == -1:
                continue
            climbDir = toonInfo[5]
            if climbDir is None or climbDir == 0:
                continue
            curT = toonInfo[1]
            ...
            if swingSeq:
                ...
                for i in xrange(len(swingSeq)):
                    ...
                    if not actorIval.isStopped():
                        ...
                        self.notify.debug('actor interval is not stopped, testFrame=%f' % testFrame)
                        if testFrame:
                            curFrame = testFrame
                            break
