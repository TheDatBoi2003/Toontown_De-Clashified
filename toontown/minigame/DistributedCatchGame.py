...
class DistributedCatchGame(DistributedMinigame):
    ...
    def exitPlay(self):
        ...
        for interval in self.dropIntervals.values():
            interval.finish()
    ...
    def getDropIval(self, x, y, dropObjName, num):
        ...
        interval = Sequence(Func(Functor(dropNode.reparentTo, render)), Parallel(Sequence(WaitInterval(self.OffscreenTime), dropIval), shadowIval), Func(cleanup), name='drop%s' % num)
        landSound = None
        if objType == Name2DropObjectType['anvil']:
            landSound = self.sndAnvilLand
        if landSound:
            interval.append(SoundInterval(landSound))
        return interval

    def startSuitWalkTask(self):
        interval = Parallel(name='catchGameMetaSuitWalk')
        ...
        while delay < CatchGameGlobals.GameDuration:
            ...
            interval.append(walkIval)

        interval.start()
        self.suitWalkIval = interval
    ...
    def getSuitWalkIval(self, startPos, stopPos, rng):
        ...
        interval = Sequence(FunctionInterval(setup), LerpPosInterval(lerpNP, duration, stopPos), FunctionInterval(cleanup))
        return interval
