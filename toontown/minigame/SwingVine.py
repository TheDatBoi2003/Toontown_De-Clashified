...
class SwingVine(NodePath):
    ...
    def startSwing(self):
        ...
        self.interval = Sequence(LerpFunctionInterval(self.swingForward, duration=self.period / 2.0, blendType='easeInOut'))
        self.interval.append(LerpFunctionInterval(self.swingBack, duration=self.period / 2.0, blendType='easeInOut'))
        self.interval.loop()
        if self.hasSpider:
    ...
    def detachToon(self, toonId):
        ...
        if toonId in self.attachedToons:
            ...
            if swingIval:
                self.notify.debug('deleting swing interval %s' % swingIval)
                swingIval.finish()
