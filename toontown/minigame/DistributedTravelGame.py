...
class DistributedTravelGame(DistributedMinigame):
    ...
    def enterMoveTrolley(self):
        ...
        if self.FlyCameraUp:
            setRightHprTime = 1.0
        cameraInterval1 = Parallel()
        cameraInterval1.append(LerpFunc(focusOnTrolley, duration - setRightHprTime, name='focusOnTrolley'))
        ...
        if self.FlyCameraUp:
            if self.FocusOnTrolleyWhileMovingUp:
                cameraInterval1.append(LerpPosInterval(camera, duration - setRightHprTime, finalPos, name='cameraMove'))
                cameraInterval2 = Sequence(LerpHprInterval(camera, setRightHprTime, finalHpr, name='cameraHpr'))
            else:
                cameraInterval2 = Sequence(LerpPosHprInterval(camera, setRightHprTime, finalPos, finalHpr, blendType='easeIn', name='cameraHpr'))
            cameraInterval = Sequence(cameraInterval1, cameraInterval2)
        else:
            cameraInterval = Sequence(cameraInterval1)
        if self.UseTrolleyResultsPanel:
            self.moveTrolleyIval.append(cameraInterval)
