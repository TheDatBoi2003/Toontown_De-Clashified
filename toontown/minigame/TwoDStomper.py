...
class TwoDStomper(DirectObject):
    ...
    def start(self, elapsedTime):
        ...
        self.interval.append(Sequence(self.getMotionIval(), Func(base.playSfx, self.stompSound, node=self.model, volume=0.3), Func(self.smoke.reparentTo, self.model), self.getSmokeTrack()))
        self.interval.append(LerpFunctionInterval(self.adjustShaftScale, duration=self.period))
        self.interval.append(LerpFunctionInterval(self.adjustCollSolidHeight, duration=self.period))
        self.interval.loop()
        self.interval.setT(elapsedTime)
        return
