from otp.otpbase import OTPBase, OTPRender
...
class ToonBase(OTPBase.OTPBase):
    ...
    def __init__(self):
        ...
        self.wantNewCamera = base.settings.getBool('camera', 'new-camera', False)
        self.CAM_SENSITIVITY = base.settings.getFloat('camera', 'mouse-sensitivity', [0.1, 0.1])
        ...
        self.SECOND_ACTION_BUTTON = 'insert'
        self.SCREENSHOT_KEY = 'f9'
        self.SPRINT = 'shift'
        self.wantCustomControls = base.settings.getBool('game', 'custom-keymap', False)
        if self.wantCustomControls:
            ...
            self.INVENTORY = keymap.get('INVENTORY', self.INVENTORY)
            self.QUESTS = keymap.get('QUESTS', self.QUESTS)
    ...
    def openMainWindow(self, *args, **kw):
        ...
        self.win.requestProperties(self.getCursorAndIcon())
        return result

    @staticmethod
    def getCursorAndIcon():
