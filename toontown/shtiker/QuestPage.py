...
class QuestPage(ShtikerPage.ShtikerPage):
    ...
    def acceptOnscreenHooks(self, on=base.QUESTS, off=base.QUESTS+'-up'):
        self.accept(on, self.showQuestsOnscreen)
        self.accept(off, self.hideQuestsOnscreen)

    def ignoreOnscreenHooks(self, on=base.QUESTS, off=base.QUESTS+'-up'):
        self.ignore(on)
        self.ignore(off)
