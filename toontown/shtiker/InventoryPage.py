...
class InventoryPage(ShtikerPage.ShtikerPage):
    ...
    def updateTrackInfo(self, trackIndex):
        ...
        if base.localAvatar.hasTrackAccess(trackIndex):
            ...
            self.trainingFrames['range'] = nextExp
            self.trainingFrames['value'] = curExp
            if curExp >= ToontownBattleGlobals.MaxSkill:
                str = TTLocalizer.InventoryPageTrackFull % trackName
                trackText = TTLocalizer.InventoryMax
                self.trainingFrames['range'] = 1
                self.trainingFrames['value'] = 1
            else:
            ...
            self.trainingFrames['text'] = trackText
            self.trainingFrames['frameColor'] = (ToontownBattleGlobals.TrackColors[trackIndex][0] * 0.6,
             ToontownBattleGlobals.TrackColors[trackIndex][1] * 0.6,
             ToontownBattleGlobals.TrackColors[trackIndex][2] * 0.6,
             1)
            self.trainingFrames['barColor'] = (ToontownBattleGlobals.TrackColors[trackIndex][0],
             ToontownBattleGlobals.TrackColors[trackIndex][1],
             ToontownBattleGlobals.TrackColors[trackIndex][2],
             1)
            self.trainingFrames.show()
        else:
    ...
    def acceptOnscreenHooks(self, on=base.INVENTORY, off=base.INVENTORY+'-up'):
        self.accept(on, self.showInventoryOnscreen)
        self.accept(off, self.hideInventoryOnscreen)

    def ignoreOnscreenHooks(self, on=base.INVENTORY, off=base.INVENTORY+'-up'):
        self.ignore(on)
        self.ignore(off)
