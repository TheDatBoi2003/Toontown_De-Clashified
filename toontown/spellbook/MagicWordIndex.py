...
class SetMaxHP(MagicWord):
    ...
    def handleWord(self, invoker, avId, toon, *args):
        ...
        if not 1 <= maxhp <= ToontownGlobals.MaxHpLimit:
            return "Can't set {}'s max laff to {}! Specify a value between 1 and {}.".format(toon.getName(), maxhp, ToontownGlobals.MaxHpLimit)
...
class ToggleTurn(MagicWord):
    aliases = ["turn"]
    desc = "Toggles turning controls."
    execLocation = MagicWordConfig.EXEC_LOC_SERVER

    def handleWord(self, invoker, avId, toon, *args):
        toon.d_setTurn()
        return "Turning has been toggled."


class SetCamera(MagicWord):
    aliases = ["camera"]
    desc = "Changes camera mode."
    execLocation = MagicWordConfig.EXEC_LOC_CLIENT
    arguments = [("FPS, Off", str, True)]

    def handleWord(self, invoker, avId, toon, *args):
        if not base.wantNewCamera:
            return "First, enable the new camera in your settings."
        ...
        if mode:
            base.localAvatar.cameraFSM.request(mode)
            return "Camera mode changed to " + mode
        else:
...
class SkipVP(MagicWord):
    ...
    def handleWord(self, invoker, avId, toon, *args):
        ...
        if battle == 'intro':
            ...
            if boss.state in ('PrepareBattleOne', 'BattleOne'):
                boss.b_setState('PrepareBattleTwo')
                return "Skipping round 1..."
            elif boss.state in ('PrepareBattleTwo', 'BattleTwo'):
                boss.b_setState('PrepareBattleThree')
                return "Skipping round 2..."
            elif boss.state in ('PrepareBattleThree', 'BattleThree'):
                boss.b_setState('Victory')
                return "Skipping final round..."
            else:
...
class GivePies(MagicWord):
    ...
    def handleWord(self, invoker, avId, toon, *args):
        ...
        if not 0 <= pieType <= ToontownBattleGlobals.WEDDING_LEVEL_INDEX:
...
class SetExp(MagicWord):
    ...
    def handleWord(self, invoker, avId, toon, *args):
        maxSkill = ToontownBattleGlobals.MaxSkill
        track = args[0]
        amt = args[1]
        ...
        if not 0 <= amt <= maxSkill:
            return "Can't set %s's experience to %s! Specify a value between 0 and %s" % (toon.getName(), amt, maxSkill)
