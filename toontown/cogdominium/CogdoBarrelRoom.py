...
class CogdoBarrelRoom:
    ...
    def showBattleAreaLight(self, visible = True):
        ...
        if lightConeNode and not lightConeNode.isEmpty():
            if visible:
                lightConeNode.show()
            else:
                lightConeNode.hide()
    ...
    def __stomperIntervals(self):
        intervals = [SoundInterval(self.stomperSfx)]
        i = 0
        for stomperDef in CogdoBarrelRoomConsts.StomperProps:
            ...
            if stomperNode:
                ...
                intervals.append(LerpPosInterval(stomperNode, CogdoBarrelRoomConsts.StomperHaltTime, Point3(0, 0, destZ), blendType='easeOut'))
            i += 1

        return Parallel(*tuple(intervals))
