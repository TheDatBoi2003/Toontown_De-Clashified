...
class CogdoMazeSuit(MazeSuit, FSM, CogdoMazeSplattable):
    ...
    def _getSuitAnimationIval(self, animName, startFrame = 0, duration = 1, partName = None, nextState = None):
        ...
        interval = Sequence(ActorInterval(self.suit, animName, startTime=startFrame / newRate, endTime=totalFrames / newRate, playRate=playRate, partName=partName))
        if nextState is not None:

            def done():
                self.request(nextState)

            interval.append(Func(done))
        return interval
