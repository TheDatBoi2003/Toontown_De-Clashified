...
class CogdoFlyingFan(CogdoFlyingObstacle):
    ...
    def startMoving(self, elapsedTime = 0.0):
        ...
        timeDelay = 0.0
        maxDur = Globals.Gameplay.FanStreamerMaxDuration
        for interval in self.streamerIvals:
            taskName = 'delayedStreamerSpinTask-fan-%i-%s' % (self.index, interval.getName())
            taskMgr.doMethodLater(timeDelay, interval.loop, taskName, extraArgs=[])
