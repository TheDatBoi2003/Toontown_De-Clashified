...
class CogdoGameAudioManager:
    ...
    def _cleanupSoundIval(self, audioSound):
        if audioSound in self._soundIvals:
            interval = self._soundIvals[audioSound]
    ...
    def _createSoundIval(self, audioSound, volume = 1.0, duration = 0.0, startTime = 0.0, source = None, register = False, cutoff = None):
        if cutoff is None:
            cutoff = self._cutoff
        interval = SoundInterval(audioSound, node=source, duration=duration, startTime=startTime, cutOff=cutoff, seamlessLoop=True, listenerNode=self._listener)
        return interval
    ...
    def playSound(self, audioSound, loop = False, source = None, playRate = 1.0, volume = 1.0):
        ...
        if source is not None and loop:
            ...
            interval = self._createSoundIval(audioSound, volume=volume, source=source)
            self._soundIvals[audioSound] = interval
            interval.loop()
        else:
