...
class DistributedCheckers(DistributedNode.DistributedNode):
    ...
    def setTimer(self, timerEnd):
        if self.fsm.getCurrentState() and self.fsm.getCurrentState().getName() == 'waitingToBegin' and not self.table.fsm.getCurrentState().getName() == 'observing':
            self.clockNode.stop()
    ...
    def setTurnTimer(self, turnEnd):
        if self.fsm.getCurrentState() and self.fsm.getCurrentState().getName() == 'playing':
            self.clockNode.stop()
    ...
    def existsLegalJumpsFrom(self, index, peice):
        if peice == 'king':
            for x in xrange(4):
                if self.board.squareList[index].getAdjacent()[x] and \
    ...
    def existsLegalMovesFrom(self, index, peice):
        if peice == 'king':
            ...
            if self.playerNum == 1:
                ...
                moveForward = [0, 3]
            for x in moveForward:
                if self.board.squareList[index].getAdjacent()[x]:
                    adj = self.board.squareList[self.board.squareList[index].getAdjacent()[x]]
                    if adj.getState() == 0:
                        return True
    ...
    def checkLegalMove(self, firstSquare, secondSquare, peice):
        ...
        if peice == 'king':
            ...
            for x in moveForward:
                if firstSquare.getAdjacent()[x] and secondSquare.getNum() in firstSquare.getAdjacent():
    ...
    def doRandomMove(self):
        ...
        while not foundLegal:
            ...
            for y in self.board.getAdjacent(self.mySquares[x]):
                if y and self.board.getState(y) == 0:
