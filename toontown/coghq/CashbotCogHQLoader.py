...
class CashbotCogHQLoader(CogHQLoader.CogHQLoader):
    ...
    def __init__(self, hood, parentFSMState, doneEvent):
        ...
        self.fsm.addState(
            State.State('mintInterior', self.enterMintInterior, self.exitMintInterior, ['quietZone', 'cogHQExterior']))
        for stateName in ['start', 'cogHQExterior', 'quietZone']:
            state = self.fsm.getStateNamed(stateName)
            state.addTransition('mintInterior')

        self.musicDict = {ToontownGlobals.CashbotHQ: 'phase_10/audio/bgm/CB_courtyard.ogg',
                          ToontownGlobals.CashbotLobby: 'phase_10/audio/bgm/CB_boss_lobby.ogg'}
        self.battleMusicFile = 'phase_10/audio/bgm/CB_courtyard_encntr.ogg'
