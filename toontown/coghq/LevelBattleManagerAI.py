...
class LevelBattleManagerAI(BattleManagerAI.BattleManagerAI):
    ...
    def __init__(self, air, level, battleCtor, battleExpAgg=None):
        BattleManagerAI.BattleManagerAI.__init__(self, air)
        self.battleCtor = battleCtor
        self.level = level
        self.battleBlockers = {}
        if battleExpAgg is None:
            battleExpAgg = BattleExperienceAggregatorAI.BattleExperienceAggregatorAI()
        self.battleExpAgg = battleExpAgg
        return
    ...
    def destroyBattleMgr(self):
        ...
        del self.battleExpAgg
        return

    def newBattle(self, cellId, zoneId, pos, suit, toonId, roundCallback=None, finishCallback=None,
        ...
        if battle:
            ...
            battle = self.battleCtor(self.air, self, pos, suit, toonId, zoneId, self.level, cellId, roundCallback,
                                     finishCallback, maxSuits)
            self.battleExpAgg.attachToBattle(battle)
