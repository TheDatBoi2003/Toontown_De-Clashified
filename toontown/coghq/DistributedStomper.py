...
class DistributedStomper(DistributedCrusherEntity.DistributedCrusherEntity):
    ...
    def startStomper(self, startTime, mode = STOMPER_START):
        ...
        if motionIval is None:
            return
        self.interval = Parallel(Sequence(motionIval, Func(self.__startSmokeTask), Func(self.sendStompToon)), name=self.uniqueName('Stomper'))
        if wantSound:
            sndDur = motionIval.getDuration()
            self.interval.append(Sequence(Wait(sndDur), Func(base.playSfx, self.sound, node=self.model, volume=0.45)))
        if self.shadow is not None and self.animateShadow:
            ...
            self.interval.append(LerpFunctionInterval(adjustShadowScale, duration=self.period))
        if mode == STOMPER_START:
            self.interval.loop()
            self.interval.setT(globalClock.getFrameTime() - self.level.startTime + self.period * self.phaseShift)
        else:
            self.interval.start(startTime)
        return
