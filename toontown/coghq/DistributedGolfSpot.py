...
from direct.interval.IntervalGlobal import Sequence, Wait, ActorInterval, Parallel, Func, LerpPosInterval, \
    ProjectileInterval, LerpScaleInterval, SoundInterval
...
class DistributedGolfSpot(DistributedObject.DistributedObject, FSM.FSM):
    ...
    def getFlyBallSplatInterval(self, x, y, z, flyBallCode, throwerId):
        ...
        interval = Parallel(Func(splat.reparentTo, render), Func(splat.setPos, x, y, z), soundIval, Sequence(ActorInterval(splat, splatName), Func(splat.detachNode)))
        return interval
