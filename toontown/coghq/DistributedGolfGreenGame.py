...
class DistributedGolfGreenGame(BattleBlocker.BattleBlocker):
    ...
    def findGrid(self, x, z, force = 0):
        ...
        for countX in xrange(self.gridDimX):
            for countZ in xrange(self.gridDimZ):
                ...
                if self.grid[countX][countZ][0] is None and testDist < currentDist and (force or self.hasNeighbor(countX, countZ)):
    ...
    def __run(self, cont = 1):
        ...
        while self.controlSprite is None and self.attackPattern:
            ...
            if self.standbySprite.holdType:
    ...
    def useTime(self, time = None):
        if time:
            ...
            if self.timerTask:
                taskMgr.remove(self.timerTaskName)
        if time and time > 0.0 and self.isActive:
