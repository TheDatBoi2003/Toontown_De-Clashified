...
class GearEntity(BasicEntities.NodePathEntity):
    ...
    def startRotate(self):
        self.stopRotate()
        try:
            intervalDur = 360.0 / self.degreesPerSec
            ...
            if intervalDur < 0.0:
                ...
                hOffset = -hOffset
            self.rotateIval = LerpHprInterval(self.model, intervalDur, Vec3(hOffset, 0, 0), startHpr=Vec3(0, 0, 0), name='gearRot-%s' % self.entId)
            self.rotateIval.loop()
            self.rotateIval.setT(globalClock.getFrameTime() - self.level.startTime + intervalDur * self.phaseShift)
