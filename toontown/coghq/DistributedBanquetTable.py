...
class DistributedBanquetTable(DistributedObject.DistributedObject, FSM.FSM, BanquetTableBase.BanquetTableBase):
    ...
    def setDinerInfo(self, hungryDurations, eatingDurations, dinerLevels, executives):
        self.dinerInfo = {}
        for i in xrange(len(hungryDurations)):
            ...
            self.dinerInfo[i] = (hungryDur, eatingDur, dinerLevel, executive)
    ...
    def createDiner(self, i):
        ...
        isExecutive=self.dinerInfo[i][3]
    ...
    def serveFood(self, food, chairIndex):
        ...
        if not food or food.isEmpty():
            ...
            foodMoveIval = Parallel(SoundInterval(self.serveFoodSfx, node=food), ProjectileInterval(food, duration=intervalDuration, startPos=food.getPos(serviceLoc), endPos=serviceLoc.getPos(serviceLoc)), LerpHprInterval(food, intervalDuration, Point3(0, -360, 0)))
