...
class LevelSuitPlannerAI(DirectObject.DirectObject):
    ...
    def __init__(self, air, level, cogCtor, battleCtor, cogSpecs, reserveCogSpecs, battleCellSpecs, battleExpAgg=None):
        self.air = air
        self.level = level
        ...
        self.battleMgr = LevelBattleManagerAI.LevelBattleManagerAI(self.air, self.level, battleCtor, battleExpAgg)
    ...
    def __genSuitObject(self, suitDict, reserve):
        ...
        if suitDict['skeleton']:
            suit.setSkelecog(1)
        if suitDict.get('name'):
            self.notify.info("This cog's name is " + str(suitDict['name']))
        suit.generateWithRequired(suitDict['zoneId'])
        suit.boss = suitDict['boss']
        suit.exe = suitDict.get('exe')
        if suit.boss or suit.exe:
            suit.b_setExecutive(1)
        else:
            suit.b_setExecutive(random.random() <= SuitBuildingGlobals.BLDG_EXE_CHANCE)
        return suit
    ...
    def requestBattle(self, suit, toonId):
        ...
        maxSuits = SuitBattleGlobals.MAX_SUIT_CAPACITY
        ...
        for otherSuit in self.battleCellId2suits[cellIndex]:
            if otherSuit is not suit:
                if self.__suitCanJoinBattle(cellIndex):
                    ...
                    if battle:
                        self.notify.warning(
                            'battle not joinable: numSuits=%s, joinable=%s, fsm=%s, toonId=%s' % (len(battle.suits),
                                                                                                  battle.isJoinable(),
