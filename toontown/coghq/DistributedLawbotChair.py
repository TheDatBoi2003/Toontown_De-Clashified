...
class DistributedLawbotChair(DistributedObject.DistributedObject, FSM.FSM):
    ...
    def loadCogJuror(self):
        ...
        level = self.randomGenerator.randrange(len(SuitDNA.suitsPerLevel)) + 1
    ...
    def enterOn(self):
        ...
        self.interval = Sequence(name=seqName)
        downAngle = -80
        for index in xrange(len(myHeadings)):
            ...
            self.interval.append(goingDown)
            self.interval.append(Wait(self.stayDownTime))
