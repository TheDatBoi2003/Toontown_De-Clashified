...
class ConveyorBelt(BasicEntities.NodePathEntity):
    ...
    def start(self):
        ...
        for i in xrange(self.numTreads):
            ...
            interval = Sequence()
            if periodsToEnd != 0:
                interval.append(LerpPosInterval(self.treads[i], duration=treadPeriod * periodsToEnd, pos=Point3(0, startY + self.numTreads * self.treadLength, 0), startPos=Point3(0, startY + i * self.treadLength, 0), fluid=1))

            def dumpContents(tread = self.treads[i]):
                tread.releaseLocalToon()

            interval.append(Sequence(Func(dumpContents), Func(self.treads[i].setPos, Point3(0, startY + self.numTreads * self.treadLength, 0))))
            if periodsFromStart != 0:
                interval.append(LerpPosInterval(self.treads[i], duration=treadPeriod * periodsFromStart, pos=Point3(0, startY + i * self.treadLength, 0), startPos=Point3(0, startY, 0), fluid=1))
            treadsIval.append(interval)
