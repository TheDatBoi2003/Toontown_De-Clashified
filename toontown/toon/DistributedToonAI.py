...
class DistributedToonAI(DistributedPlayerAI.DistributedPlayerAI, DistributedSmoothNodeAI.DistributedSmoothNodeAI, PetLookerAI.PetLookerAI):
    ...
    def b_setTrainingFrames(self, frames):
        self.setTrainingFrames(frames)
        self.d_setTrainingFrames(frames)

    def d_setTrainingFrames(self, frames):
        self.sendUpdate('setTrainingFrames', [frames])

    def setTrainingFrames(self, frames):
        self.trainingFrames = frames
        self.__calcTracks()
    ...
    def __calcTracks(self, track=None, refund=None):
        ...
        for frame in self.trainingFrames:
            if frame >= 0:
                trackCounts[frame] += 1
                if trackCounts[frame] > 3:
                    self.notify.debug('Too many frames! Removing this one.')
                    frame = -1
                    trackCounts[frame] -= 1
        trackAccess = [i/2 for i in trackCounts]
        trackPrestige = [i/3 for i in trackCounts]

        for i in xrange(len(trackAccess)):
            if not trackAccess[i]:
                self.inventory.zeroTrack(i)
        self.b_setInventory(self.getInventory())
    ...
    def setEmoteAccess(self, bits):
        if len(bits) == 20:
            ...
            self.notify.warning('New emote access list must be the same size as the old one: %s vs. %s '
                                % (len(bits), len(self.emoteAccess)))
            return
    ...
    def takeMoney(self, deltaMoney, bUseBank = True):
        ...
        if deltaMoney > totalMoney:
            ...
            return False
        if bUseBank:
            if self.bankMoney < deltaMoney:
                deltaPocketMoney = deltaMoney - self.bankMoney
                self.b_setBankMoney(0)
            else:
                self.b_setBankMoney(self.bankMoney - deltaMoney)
                deltaPocketMoney = 0
        else:
            deltaPocketMoney = deltaMoney

        self.b_setMoney(self.money - deltaPocketMoney)
        return True
    ...
    def b_setPieType(self, pieType, pieVelocity=0):
        self.setPieType(pieType, pieVelocity)
        self.d_setPieType(pieType, pieVelocity)

    def d_setPieType(self, pieType, pieVelocity):
        self.sendUpdate('setPieType', [pieType, pieVelocity])

    def setPieType(self, pieType, pieVelocity):
        self.pieType = pieType
        self.pieVelocity = pieVelocity
    ...
    def b_setTrackPrestige(self, trackArray):
        self.setTrackPrestige(trackArray)
        self.d_setTrackPrestige(trackArray)

    def d_setTrackPrestige(self, trackArray):
        self.sendUpdate('setTrackPrestige', [trackArray])
