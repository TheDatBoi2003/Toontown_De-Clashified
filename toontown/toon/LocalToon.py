...
class LocalToon(DistributedToon.DistributedToon, LocalAvatar.LocalAvatar):
    ...
    def initInterface(self):
        ...
        self.accept(base.SECOND_ACTION_BUTTON, self.__tossZeroPie)
        ...
        if base.settings.getBool('game', 'jump-cancels-pie', False):
    ...
    if base.wantKarts:
        ...
        def addKartPage(self):
            if self.hasKart():
                if hasattr(self, 'kartPage') and self.kartPage:
                    return
    ...
    def __tossZeroPie(self):
        self.__beginTossPie(0)
        self.__endTossPie(0, fixedPower=-1)

    def __beginTossPie(self, time):
        if self.tossPieStart:
            return
    ...
    def __endTossPie(self, time, fixedPower=0):
        if self.tossPieStart is None:
            return
        taskName = self.uniqueName('updatePiePower')
    ...
    def localPresentPie(self, time):
        ...
        interval = self.getPresentPieInterval(pos[0], pos[1], pos[2], hpr[0], hpr[1], hpr[2])
        interval = Sequence(interval, name=self.uniqueName('localPresentPie'))
        self.tossTrack = interval
        interval.start()
    ...
    def makePiePowerMeter(self):
        ...
        if self.__piePowerMeter is None:
    ...
    def __startMoveFurniture(self):
        ...
        if self.cr.furnitureManager:
            self.cr.furnitureManager.d_suggestDirector(self.doId)
        elif self.furnitureManager:
    ...
    def setFurnitureDirector(self, avId, furnitureManager):
        if avId == 0:
            ...
            if self.furnitureManager is None or self.furnitureDirector != avId:
    ...
    def getAvPosStr(self):
        ...
        if hasattr(base.cr.playGame.hood, 'loader') and hasattr(base.cr.playGame.hood.loader, 'place') and base.cr.playGame.getPlace():
    ...
    def addGolfPage(self):
        if self.hasPlayedGolf():
            if hasattr(self, 'golfPage') and self.golfPage:
                return
            if not launcher.getPhaseComplete(6):
    ...
    def addEventsPage(self):
        if hasattr(self, 'eventsPage') and self.eventsPage:
            return
    ...
    def createSystemMsgAckGui(self):
        if self.systemMsgAckGui is None or self.systemMsgAckGui.isEmpty():
            message = 'o' * 100
    ...
    def hideSystemMsgAckGui(self):
        if self.systemMsgAckGui and not self.systemMsgAckGui.isEmpty():
            self.systemMsgAckGui.hide()
        return
