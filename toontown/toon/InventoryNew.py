...
class InventoryNew(InventoryBase.InventoryBase, DirectFrame):
    ...
    def __init__(self, toon, invStr=None, ShowSuperGags=1):
        ...
        self.activeTab=4
        self.propAndOrganicBonusStack=0
    ...
    def uberGagToggle(self, showSuperGags=1):
        ...
        for itemList in self.invModels:
            for itemIndex in xrange(MAX_LEVEL_INDEX + 1):
                if itemIndex <= 7 + 1 or self.showSuperGags:
    ...
    def enableUberGags(self, enableSG=-1):
        ...
        for buttonList in self.buttons:
            for buttonIndex in xrange(8 + 1, MAX_LEVEL_INDEX + 1):
    ...
    def load(self):
        ...
        bars=[self.healRow, self.trapRow, self.lureRow, self.soundRow, self.throwRow, self.squirtRow, self.zapRow,
              self.dropRow]
        ...
        trashcanGui.find('**/TrashCan_RLVR')), text=('', TTLocalizer.InventoryDelete, TTLocalizer.InventoryDelete),
                                            ...
                                            relief=None, pos=(-1.3, 0, -0.35), scale=0.70)
        self.deleteAllButton=DirectButton(parent=self.invFrame, image=(
        trashcanGui.find('**/TrashCan_CLSD'), trashcanGui.find('**/TrashCan_OPEN'),
        trashcanGui.find('**/TrashCan_RLVR')),
                                          text=('', "Delete All", "Delete All"),
                                          text_fg=(1, 0, 0, 1), text_shadow=(1, 1, 1, 1), text_scale=0.1,
                                          text_pos=(0, -0.1), text_font=getInterfaceFont(), textMayChange=0,
                                          relief=None, pos=(0.55, 0, -1), scale=0.75, command=self.__zeroInvConfirm)
        ...
        for track in xrange(len(Tracks)):
            trackTab=DirectButton(parent=self.invFrame, text=TextEncoder.upper(Tracks[track]), relief=None,
                                  scale=(0.5, 0.5, 0.5), pos=(-0.7 + offset, 0, -0.54), geom=self.tab,
                                  geom_color=(TrackColors[track][0] * 0.6,
                                              TrackColors[track][1] * 0.6,
                                              TrackColors[track][2] * 0.6,
                                              1), text_font=ToontownGlobals.getInterfaceFont(),
                                  text_scale=TTLocalizer.INtrackNameLabels, text_pos=(0, 0, 0),
                                  text_align=TextNode.ACenter, command=self.doTab, extraArgs=[track], pressEffect=1)
            offset+=0.2
            trackFrame=DirectFrame(parent=self.invFrame, image=bars[track], scale=(1.0, 1.0, 1.0), pos=(0, 0, -0.7),
                                   state=DGG.NORMAL, relief=None)
            ...
            adjustLeft=-0.065
            self.trackNameLabels.append(DirectLabel(text=TextEncoder.upper(Tracks[track]), parent=self.trackRows[track],
                                                    pos=(-0.72 + adjustLeft, -0.1, 0.01),
                                                    scale=TTLocalizer.INtrackNameLabels, relief=None,
                                                    text_fg=(0.2, 0.2, 0.2, 1), text_font=getInterfaceFont(),
                                                    text_align=TextNode.ALeft, textMayChange=0))
            self.trackBars.append(
                DirectWaitBar(parent=self.trackRows[track], pos=(-0.58 + adjustLeft, -0.1, -0.025), relief=DGG.SUNKEN,
                              frameSize=(-0.6,
                                         0.6,
                                         -0.1,
                                         0.1), borderWidth=(0.02, 0.02), scale=0.25,
                              frameColor=(TrackColors[track][0] * 0.6,
                                          ...
                                          1), barColor=(TrackColors[track][0] * 0.9,
                                                        ...
                                                        1), text='0 / 0', text_scale=0.16, text_fg=(0, 0, 0, 0.8),
            ...
            for item in xrange(0, len(Levels)):
                button=DirectButton(parent=self.trackRows[track], image=(self.upButton,
                                                                         ...
                                                                         self.rolloverButton,
                                                                         self.flatButton),
                                    geom=self.invModels[track][item], text='50', text_scale=0.04,
                                    text_align=TextNode.ARight, geom_scale=0.7, geom_pos=(-0.01, -0.1, 0),
                                    text_fg=Vec4(1, 1, 1, 1), text_pos=(0.07, -0.04), textMayChange=1, relief=None,
        ...
        self.circleFrame=DirectFrame(parent=self.detailFrame, relief=None, image=self.battleCircle, image_scale=0.25,
                                     pos=(0, 0, -0.25))
        self.detailNameLabel=DirectLabel(parent=self.circleFrame, text='', text_scale=TTLocalizer.INdetailNameLabel,
                                         text_fg=(0.05, 0.14, 0.4, 1), scale=0.045, pos=(0, 0, 0.1),
                                         text_font=getInterfaceFont(), text_align=TextNode.ACenter, relief=None,
                                         image=self.invModels[0][0])
        self.detailAmountLabel=DirectLabel(parent=self.circleFrame, text='', text_fg=(0.05, 0.14, 0.4, 1), scale=0.04,
                                           pos=(0, 0, -0.07), text_font=getInterfaceFont(), text_align=TextNode.ACenter,
                                           relief=None)
        self.detailDataLabel=DirectLabel(parent=self.circleFrame, text='', text_fg=(0.05, 0.14, 0.4, 1), scale=0.03,
                                         pos=(0, 0, -0.1), text_font=getInterfaceFont(), text_align=TextNode.ACenter,
                                         relief=None)
        self.detailCreditLabel=DirectLabel(parent=self.circleFrame, text=TTLocalizer.InventorySkillCreditNone,
                                           text_fg=(0.05, -0.14, -0.2, 1), scale=0.04, pos=(0, 0, 0.05),
                                           text_font=getInterfaceFont(), text_align=TextNode.ACenter, relief=None)
        self.detailCreditLabel.hide()
        self.totalLabel=DirectLabel(text='', parent=self.circleFrame, pos=(0, 0, 0.02), scale=0.05,
    ...
    def doTab(self, index):
        ...
        for track in xrange(len(self.trackRows)):
            ...
            self.trackTabs[track]['geom_color']=Vec4(TrackColors[track][0] * 0.6, TrackColors[track][1] * 0.6,
    ...
    def __zeroInvConfirm(self):
        self.cleanupDialog()
        self.dialog=TTDialog.TTDialog(style=TTDialog.YesNo, text=TTLocalizer.InventoryDeleteConfirm,
    ...
    def showDetail(self, track, level, event=None):
        ...
        if self.propAndOrganicBonusStack:
            ...
            if damageBonus:
                damageBonusStr=TTLocalizer.InventoryDamageBonus % damageBonus
        accString=TTLocalizer.BattleGlobalAvTrackAccStringsNEW[track]
                ...
                for level in xrange(len(Levels[track])):
                    ...
                    if self.itemIsUsable(track, level):
                        ...
                        if self.numItem(track, level) >= self.getMax(track,
                                                                     level) or totalProps == maxProps or unpaid and gagIsPaidOnly(
                                track, level) or level > LAST_REGULAR_GAG_LEVEL:
    ...
    def getExtraText(self, track, level, organicBonus=False):
        if track == SQUIRT_TRACK:
            if organicBonus:
                ...
                bonusRounds=0
            if bonusRounds:
                text=TTLocalizer.InventorySquirtRoundsString % str(NumRoundsSoaked[level]) + ' (+1)'
            else:
                text=TTLocalizer.InventorySquirtRoundsString % str(NumRoundsSoaked[level])
            return text
    ...
    def loadBattleFrame(self):
        ...
        self.levelsButton=DirectButton(self, relief=None, pos=(0.675, 0, -0.5), text='',
                                       text_scale=TTLocalizer.INlevelsButton, text_fg=Vec4(1, 1, 1, 1), textMayChange=1,
                                       image=(self.upButton, self.downButton, self.rolloverButton),
                                       image_scale=(2.5, 1.05, 1), image_color=(1, 0.6, 0, 1),
                                       command=self.__handleLevels)
        self.battleFrame=DirectFrame(relief=None, parent=self)
        self.runButton=DirectButton(parent=self.battleFrame, relief=None, pos=(1.4, 0, -0.5),
                                    text=TTLocalizer.InventoryRun, text_scale=TTLocalizer.INrunButton,
                                    text_pos=(0, -0.02), text_fg=Vec4(1, 1, 1, 1), textMayChange=0,
                                    image=(self.upButton, self.downButton, self.rolloverButton),
                                    image_scale=(2, 1.05, 1), image_color=(0, 0.6, 1, 1), command=self.__handleRun)
        self.sosButton=DirectButton(parent=self.battleFrame, relief=None, pos=(1.45, 0, -0.7),
                                    ...
                                    image=(self.upButton, self.downButton, self.rolloverButton),
                                    image_scale=(2, 1.05, 1), image_color=(0, 0.6, 1, 1), command=self.__handleSOS)
        self.passButton=DirectButton(parent=self.battleFrame, relief=None, pos=(1.45, 0, -0.6),
                                     text=TTLocalizer.InventoryPass, text_scale=TTLocalizer.INpassButton,
                                     text_pos=(0, -0.02), text_fg=Vec4(1, 1, 1, 1), textMayChange=1,
                                     image=(self.upButton, self.downButton, self.rolloverButton),
                                     image_scale=(2, 1.05, 1), image_color=(0, 0.6, 1, 1), command=self.__handlePass)
        self.fireButton=DirectButton(parent=self.battleFrame, relief=None, pos=(1.4, 0, -0.8),
                                     text=TTLocalizer.InventoryFire, text_scale=TTLocalizer.INfireButton,
                                     text_pos=(0, -0.02), text_fg=Vec4(1, 1, 1, 1), textMayChange=0,
                                     image=(self.upButton, self.downButton, self.rolloverButton),
                                     image_scale=(2, 1.05, 1), image_color=(0, 0.6, 1, 1), command=self.__handleFire)
        self.tutText=DirectFrame(parent=self.battleFrame, relief=None, pos=(-1, 0, -0.1133), scale=0.143,
                                 image=DGG.getDefaultDialogGeom(), image_scale=5.125, image_pos=(0, 0, -0.65),
                                 image_color=ToontownGlobals.GlobalDialogColor, text_scale=TTLocalizer.INclickToAttack,
                                 text=TTLocalizer.InventoryClickToAttack, textMayChange=0)
        self.tutText.hide()
