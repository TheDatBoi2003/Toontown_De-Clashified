...
class DistributedToon(DistributedPlayer.DistributedPlayer, Toon.Toon, DistributedSmoothNode.DistributedSmoothNode, DelayDeletable):
        ...
        if hoodId != 0:
            ...
            if hoodId == 1:
                if currentHoodId == ToontownGlobals.ToontownCentral:
                    effect = CENormal
            elif currentHoodId and currentHoodId != hoodId:
                effect = CENormal
        if self.ghostMode:
            effect = CEGhost
    ...
    def presentPie(self, x, y, z, h, p, r, timestamp32):
        ...
        interval = self.getPresentPieInterval(x, y, z, h, p, r)
        if ts > 0:
            startTime = ts
            lastTossTrack.finish()
        else:
            interval = Sequence(Wait(-ts), interval)
            lastTossTrack.finish()
            startTime = 0
    ...
    def cleanupPieInHand(self, endAnimation=1):
        if self.tossTrack and endAnimation:
