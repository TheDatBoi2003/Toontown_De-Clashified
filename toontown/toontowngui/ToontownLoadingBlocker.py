...
class ToontownLoadingBlocker(TTDialog.TTDialog):
    ...
    def __shrinkLoadingBar(self):
        if self.__isValidDownloadBar():
            ...
            barPosIval = LerpPosInterval(self.bar, intervalDuration, (-0.81, 0, -0.96))
            barScaleIval = LerpScaleInterval(self.bar, intervalDuration, (0.25, 0.25, 0.25))
            ...
            textScaleIval = LerpFunc(scaleText, fromData=0.08, toData=0.16, duration=intervalDuration)
            textPosIval = LerpFunc(posText, fromData=-0.035, toData=-0.05, duration=intervalDuration)
